                            ...
                            cols: [
                                {
                                    ...
                                    multiple: false,
                                    link: "mylist",
                                    upload: "/aoz/wr/imp/uploadFileTov",
                                    on: {
...
function loadDataTree() {
    get_ajax('/aoz/wr/dic?name=productsList', 'GET', {}, function (gson) {
        if (gson) {
...
function saveDataValidate(formName) {
    ...
    if (form.getValues().code.length == 11) {
        if (form.validate()) {
            ...
            get_ajax('/aoz/wr/dic/saveProducts', 'POST', json, function (gson) {
                if (!gson.result) {
...
function removeProduct(formName, btn) {
    ...
    get_ajax('/aoz/wr/dic/removeProducts', 'POST', form.getValues().id, function (gson) {
        if (!gson.result) {
            messageBox('Ошибка', gson.message);
            btn.enable();
            return;
