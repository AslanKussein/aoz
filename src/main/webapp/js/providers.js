...
function saveDataValidate(formName) {
    ...
    if (form.validate()) {
        ...
        get_ajax('/aoz/wr/dic/saveProviders', 'POST', json, function (gson) {
            if (!gson.result) {
...
function removeProviders(formName, btn) {
    var form = $$(formName);
    get_ajax('/aoz/wr/dic/removeProviders', 'GET', {id: form.getValues().id}, function (gson) {
        if (!gson.result) {
            messageBox('Ошибка', gson.message);
            btn.enable();
            return;
