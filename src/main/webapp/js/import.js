...
function createImportAccordion(gson) {
    for (var k in gson) {
            ...
            body: {
                rows: [
                    ...
                    {height: 10},
                    {
                        ...
                        select: "row",
                        url: '/aoz/wr/app/getImportList?provider=' + gson[k].companyName +'&impDate=' + gson[k].importDate,
                        datafetch: 15,
                        pager: {
        ...
        $$("srcProducts"+k).attachEvent("onSearchIconClick", function (e) {
            searchProducts("productTable" + k, '/aoz/wr/app/getImportList?provider=' + gson[k].companyName +'&impDate=' + gson[k].importDate+'&srcText='+this.getValue());
        });
