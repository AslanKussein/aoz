        ...
        cols: [
            {
                ...
                rows: [

                    {
                        ...
                        autoheight: true,
                        scroll: false,
                        url: "/aoz/wr/user/getUsersDetailList",
                        columns: [
...
function createUser(btn) {
    ...
    if (form.validate()) {
        ...
        get_ajax('/aoz/wr/admin/createUser', 'POST', {
            uName: json.uName,
...
function getGroupMembersByUName(uName) {
    get_ajax('/aoz/wr/user/getGroupMembersByUName', 'GET', {
        uName: uName
    }, function (gson) {
        if (gson) {
            var uRoles = [];
...
function submitEditUserForm() {
    ...
    get_ajax('/aoz/wr/user/editUser', 'POST', {json: JSON.stringify(json)}, function (gson) {
        if (!gson || !gson.result) {
            messageBox('Ошибка', gson.message);
        } else {
            $$("userDetailTable").parse(json);
...
function lockUser(item, lock) {
    ...
    get_ajax('/aoz/wr/admin/lockUser', 'POST', {uName: item.uName, lock: lock}, function (gson) {
        if (!gson.result) {
            messageBox('Ошибка', gson.message);
            return;
        }
